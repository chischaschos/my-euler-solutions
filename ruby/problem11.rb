...
def values elements, row_mover, column_mover
  row = 1
  line = []
  4.times do |index|
...
def value elements, row, column
  elements.fetch(row, []).fetch(column, 0).to_i
end

def max_line numbers
  result = 0
  line = []
  numbers.size.times do |row|
    row.size.times do |column|
      up_sum, up_line = values numbers, ->(index){row - index}, ->(index){column}

      down_sum, down_line = values numbers, ->(index){row + index}, ->(index){column}

      left_sum, left_line = values numbers, ->(index){index}, ->(index){column - index}
      ...
      result < left_sum && (result = left_sum) && line = left_line
      result < right_sum && (result = right_sum) && line = right_line
      result < diagonal_sum && (result = diagonal_sum) && line = diagonal_line
      result < inv_diagonal_sum && (result = inv_diagonal_sum) && line = inv_diagonal_line
    end
  end
